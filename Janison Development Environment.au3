...
Local $app_name = "Janison Development Environment"
...
$storage_format = '<a href=\"https://janisoncls.atlassian.net/wiki/download/attachments/494207048/Janison%20Development%20Environment%20portable.exe\">Click to update page</a><br /><br />'
...
Local $deployment = _OctopusGetLatestTenantIdsDeploymentIdTaskIdCreatedForEnvironment("Environments-24")
...
for $deployment_num = 0 to (UBound($deployment) - 1)
	...
	_Toast_Show(0, $app_name, "Getting deployment ID " & $deployment_id & " tenant ID " & $tenant_id, -300, False, True)
...
for $subscription_num = 0 to (UBound($subscription_arr) - 1)
	...
	$ps_script_arr[1] = "Get-AzureRmResource -ResourceType 'Microsoft.Sql/servers' -ResourceGroupName '*-DEV*'"
...
Update_Confluence_Page("https://janisoncls.atlassian.net", "JAST", "495845841", "495682177", "Development Environment", $storage_format)
